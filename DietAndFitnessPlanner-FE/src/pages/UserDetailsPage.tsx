...
const HEIGHT_FEET_OPTIONS = [3, 4, 5, 6, 7, 8];
const HEIGHT_INCH_OPTIONS = Array.from({ length: 12 }, (_, index) => index);
const WEIGHT_OPTIONS = Array.from({ length: 35 }, (_, index) => 30 + index * 5);
...
const UserDetailsPage = () => {
  ...
  const handleMultiSelectChange =
    (field: "injury_notes" | "avoid_exercises" | "equipment_access") =>
  ...
  const validate = () => {
    ...
    if (formData.targetWeight && (!Number.isFinite(targetWeight) || targetWeight < 25 || targetWeight > 300)) {
      next.targetWeight = "Target weight must be between 25 and 300 kg";
    }
  ...
  const isFormComplete = useMemo(
    () => {
      const requiredFields: Array<"name" | "email" | "age" | "heightFeet" | "heightInches" | "weight" | "fitnessLevel" | "goal" | "dietType" | "budget_preference"> = [
        "name",
        "email",
        "age",
        "heightFeet",
  ...
  const handleGenerate = async () => {
    ...
    setIsLoading(true);
    const injuryNotes = formData.injury_notes;
    const avoidExercises = formData.avoid_exercises;
    const equipmentAccess = formData.equipment_access;
    const dataToSend = {
      ...formData,
      injury_notes: injuryNotes.join(", "),
      avoid_exercises: avoidExercises.join(", "),
      equipment_access: equipmentAccess.join(", "),
      injuryNotes,
      avoidExercises,
      equipmentAccess,
      ...
      targetWeight: formData.targetWeight ? parseInt(formData.targetWeight, 10) : undefined,
    };
          ...
          <Stack spacing={2.2}>
            ...
            >
              ...
              <TextField
                ...
                helperText={errors.heightFeet || "Pick from the list"}
                sx={getInputSx()}
              >
              ...
              <TextField
                ...
                helperText={errors.heightInches || "Pick from the list"}
                sx={getInputSx()}
              >
              ...
              <TextField
                ...
                helperText={errors.targetWeight || "Optional, but helps personalize your goal"}
                sx={getInputSx()}
              />
            </Box>
            ...
            <TextField
              ...
              onChange={handleChange}
              error={!!errors.weight}
              helperText={errors.weight || "Choose the closest match"}
              sx={getInputSx()}
            >
            ...
            >
              <Autocomplete
                ...
                onChange={handleMultiSelectChange("injury_notes")}
                disableCloseOnSelect
                renderInput={(params) => (
                  <TextField
                    {...params}
                    label="Injury or limitation notes"
                    helperText={errors.injury_notes || "Pick one or more common injury types"}
                    error={!!errors.injury_notes}
                    sx={getInputSx()}
                  />
                )}
              />
              <Autocomplete
                multiple
                options={AVOID_EXERCISE_OPTIONS}
                value={formData.avoid_exercises}
                onChange={handleMultiSelectChange("avoid_exercises")}
                disableCloseOnSelect
                renderInput={(params) => (
                  <TextField
                    {...params}
                    label="Exercises to avoid"
                    helperText={errors.avoid_exercises || "Choose the movements you want to skip"}
                    error={!!errors.avoid_exercises}
                    sx={getInputSx()}
                  />
                )}
              />
            ...
            <Autocomplete
              ...
              onChange={handleMultiSelectChange("equipment_access")}
              disableCloseOnSelect
              renderInput={(params) => (
                <TextField
                  {...params}
                  label="Equipment available"
                  helperText={errors.equipment_access || "Pick the equipment you actually have access to"}
